...
void write_curve_csv(const std::string& curve_file, const ICurve& curve)
{
    Vector point, velocity, acceleration;
    ...
    for (double t = 0.0; t <= 1.0; t += 1.0 / CURVE_RESOLUTION)
    {
        ...
        velocity = curve.get_velocity(t);
        acceleration = curve.get_acceleration(t);
...
HermiteCubicCurve create_test_hermite_cubic_curve()
{
    ...
    Vector v0 = {0.0, 3.0};
    Vector v1 = {0.0, 3.0};
    return HermiteCubicCurve(p0, v0, p1, v1);
}

void test_hermite_cubic()
{
    write_curve_csv("test/curves/hermite_cubic.csv", create_test_hermite_cubic_curve());
}